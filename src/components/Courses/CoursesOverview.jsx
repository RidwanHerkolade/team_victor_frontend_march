...
// Helper function to convert filter name to URL slug
  ...
  return (
    <section className='min-h-screen w-full bg-[#f5f5f7] px-4 py-3'>
      <div className='w-full'>
            ...
            >
              ...
              <div className='flex min-h-[158px] flex-col'>
                <div className='flex-1 border-t border-[#d9dde4] px-5 pb-3 pt-4'>
                  <h3 className='text-xl font-bold leading-[1.4] text-[#0F172A]'>{course.title}</h3>

                  <span
                    className={`mt-3 inline-flex rounded-md px-2.5 py-1 text-[11px] font-medium ${
                      levelStyles[course.level] ?? "bg-gray-100 text-gray-500"
                    }`}
                  >
                    {course.level}
                  </span>
                ...
                {course.progress !== null && course.progress !== undefined ? (
                  <div className='border-t border-[#d9dde4] px-5 py-4'>
                    <div className='flex items-center justify-between gap-3'>
                      <div className='flex-1'>
                        {/* changed progress to number */}
                        <p className='mb-2 text-[12px] text-[#7b8494]'>{course.progress}% Completed</p>
                        <div className='h-[6px] w-full rounded-full bg-[#ececf1]'>
                          <div className='h-full rounded-full bg-[#7c3aed]' style={{ width: `${course.progress}%` }} />
                        </div>
                      </div>
                  ...
                  <div className='border-t border-[#d9dde4] px-5 py-4'>
                    <div className='flex items-center gap-3'>
                      <img src={course.img} alt={course.author} className='h-9 w-9 rounded-full object-contain' />
                      <div className='leading-tight'>
                        <p className='text-[13px] font-medium text-[#374151]'>{course.author}</p>
                        <p className='mt-1 text-[11px] text-[#7b8494]'>{course.enrolled}</p>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {filteredCourses.length === 0 && <p className='mt-10 text-center text-gray-500'>No courses found</p>}
      </div>
    </section>
  );
}
